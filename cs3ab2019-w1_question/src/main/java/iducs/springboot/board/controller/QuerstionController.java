...
public class QuerstionController {
	...
	@GetMapping("/writer")
	public String getAllUsers(Model model, HttpSession session, String name) {
		List<Question> questions = questionService.getQuestionsByUser(name);
		model.addAttribute("questions", questions);
		return "/questions/list"; 
	}	
	...
	public String updateQuestionById(@PathVariable(value = "id") Long id, String title, String contents, Model model, HttpSession session) {
		User sessionUser = (User) session.getAttribute("user");
		Question updateQuestion = new Question(id, title, sessionUser, contents);		
